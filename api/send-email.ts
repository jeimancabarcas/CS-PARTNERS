...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // CORS configuration to allow Vercel domain and your custom domain
  ...
  if (req.method !== "POST") {
    return res.status(405).json({ error: "Mehtod not allowed. Use POST." });
  }
  ...
  try {
    ...
    // HTML Email body matching the corporate styling
    const htmlBody = `
    ...
    console.error("Vercel Serverless Function Error sending email:", error);
